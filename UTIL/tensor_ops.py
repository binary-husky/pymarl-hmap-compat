...
class ConfigCache(object):
    ...
    def read_cfg(self):
        ...
        if GlobalConfig.cfg_ready:
            ...
            self.init = True
            self.pytorch_gpu_init(GlobalConfig)

    def pytorch_gpu_init(self, cfg):
        import torch, os
...
if ENABLE_SYNC:
    ...
    def get_hash_pure(x):
        ...
        #     return md5.hexdigest()
        if hasattr(x, "cpu"):
            buf = x.detach().cpu().numpy()
            md5.update(buf.data.tobytes())
            return md5.hexdigest(), buf
        elif hasattr(x, "numpy"):
            buf = x.numpy()
            md5.update(buf.data.tobytes())
            return md5.hexdigest(), buf
            ...
            md5.update(x.data.tobytes())
            return md5.hexdigest(), x
        else:
            try:
                md5.update(x.encode("utf-8"))
                return md5.hexdigest(), x.encode("utf-8")
            except:
                return str(x), x
    import pickle, os
    # mod = 'lead'
    ...
    def sychronize_experiment(key, data, reset_when_close=False):
        if mod == 'lead':
            hash_code, pure_data = get_hash_pure(data)
        ...
        if mod == 'follow':
            ...
            target = sychronize_internal_hashdict[key][follow_cnt[key]]
            if hash_code != target['hash_code']:
                if isinstance(pure_data, np.ndarray) and (np.isclose(target['data'], pure_data).all()):
